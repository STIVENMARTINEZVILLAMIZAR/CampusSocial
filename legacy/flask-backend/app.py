...
def social_generate():
    ...
    preview = {
        ...
        'body': result.get('linkedin') or result.get('body'),
        'hashtags': result.get('hashtags', []),
    }

    # Si persist=false (p. ej. borrador guardado en Firebase), no escribir drafts.json
    if data.get('persist') is False:
        return jsonify({
            'success': True,
            'preview': preview,
            'generated': {
...
def social_schedule():
    ...
    Acepta borrador local (draft_id) o contenido inline para clientes Firebase."""
    if not drafts_storage:
        return jsonify({'success': False, 'error': 'Storage no disponible'}), 500
    ...
    if not draft:
        content_inline = data.get('content')
        if not content_inline:
            return jsonify({'success': False, 'error': 'Borrador no encontrado o falta content (modo Firebase)'}), 404
        draft = {
            'topic': data.get('topic', ''),
            'body': content_inline,
            'linkedin': content_inline,
            'edited_body': content_inline,
        }
        draft_id = draft_id or data.get('firebase_draft_id') or 'firebase-inline'
    ...
    updated_local = None
    if drafts_storage.get_draft(draft_id):
        updated_local = drafts_storage.update_draft(draft_id, {
            'status': 'programado',
            'schedule_at': schedule_at,
            'platforms': platforms,
            'content': content,
    ...
    return jsonify({
        ...
        'draft': updated_local,
        'n8n': n8n_response,
        'message': 'Publicación programada' if webhook_url else 'Solicitud registrada (sin webhook n8n); actualiza estado en Firebase',
    }), 200
